...
<!ENTITY % th.words
  PUBLIC "-//Norman Walsh//ENTITIES DocBook Stylesheet Localization//TH"
         "dbl1th.ent">
%th.words;
]>
...
(define (th-author-string #!optional (author (current-node)))
...
(define (gentext-th-xref-strings gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (xref   (assoc name (th-xref-strings))))
    (if xref
	(car (cdr xref))
...
(define (gentext-th-element-name gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
...
(define (local-th-intra-label-sep)
  (list))

(define (th-intra-label-sep)
  (list
...
(define (gentext-th-intra-label-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-th-intra-label-sep)))
	 (sep    (assoc name (th-intra-label-sep))))
    (if lsep
        (car (cdr lsep))
        (if sep
            (car (cdr sep))
...
(define (local-th-label-title-sep)
  (list))

(define (th-label-title-sep)
  (list
...
(define (gentext-th-label-title-sep gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (lsep   (assoc name (local-th-label-title-sep)))
	 (sep    (assoc name (th-label-title-sep))))
    (if lsep
      (car (cdr lsep))
      (if sep
  	  (car (cdr sep))
...
(define (th-label-number-format-list)
  (list
...
(define (th-label-number-format gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (format (assoc name (th-label-number-format-list))))
    (if format
	(car (cdr format))
	"1")))
...
(define ($lot-title-th$ gind)
  (let* ((giname (if (string? gind) gind (gi gind)))
	 (name   (normalize giname))
	 (title  (assoc name (th-lot-title))))
    (if title
	(car (cdr title))
...
(define %gentext-th-start-quote% (dingbat "ldquo"))

(define %gentext-th-end-quote% (dingbat "rdquo"))

(define %gentext-th-start-nested-quote% (dingbat "lsquo"))

(define %gentext-th-end-nested-quote% (dingbat "rsquo"))

(define %gentext-th-by% "&by;") ;; e.g. Copyright 1997 "by" A. Nonymous
                           ;; Authored "by" Jane Doe

(define %gentext-th-edited-by% "&Editedby;")
                           ;; "Edited by" Jane Doe

(define %gentext-th-revised-by% "&Revisedby;")
                           ;; "Revised by" Jane Doe

(define %gentext-th-page% "")

(define %gentext-th-and% "&and;")

(define %gentext-th-listcomma% "&listcomma;")

(define %gentext-th-lastlistcomma% "&lastlistcomma;")

(define %gentext-th-bibl-pages% "&Pgs;")

(define %gentext-th-endnotes% "&Notes;")

(define %gentext-th-table-endnotes% "&TableNotes;:")

(define %gentext-th-index-see% "&See;")
...
(define (gentext-th-nav-next next)
  (make sequence (literal "&nav-next;")))

(define (gentext-th-nav-up up)
  (make sequence (literal "&nav-up;")))

(define (gentext-th-nav-home home)
